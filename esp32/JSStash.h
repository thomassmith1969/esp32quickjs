...
// Values are stored on global.__stashed_objects[uint32 key] — intermediate hidden object.
  static SemaphoreHandle_t stashMutex_ = xSemaphoreCreateMutex();
...
static uint32_t JSStash_stash(JSContext* ctx, JSValue value) {
    ...
    static JSValue JSStash_get(JSContext* ctx, uint32_t id) {
    ...
    static void JSStash_release(JSContext* ctx, uint32_t id) {
        if (id == 0) return;
